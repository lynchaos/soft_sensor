...
def generate_bioreactor_data(n_samples=500):
    """
    Simulate a fed-batch CHO culture with realistic dynamics
    """
...
# Generate data
df = generate_bioreactor_data(n_samples=500)
...
ax4.plot(X_test[test_indices, 0], y_test[test_indices], 'o-',
         label='Measured (offline)', linewidth=2, markersize=6, alpha=0.7)
ax4.plot(X_test[test_indices, 0], y_test_pred[test_indices], 's-',
...
bars = ax5.barh(feature_names, sensitivities, color=colors, edgecolor='black', linewidth=1.5)
...
plt.tight_layout()
plt.savefig('bioreactor_soft_sensor.png', dpi=300, bbox_inches='tight')
print("\nFigure saved as 'bioreactor_soft_sensor.png'")
plt.show()
